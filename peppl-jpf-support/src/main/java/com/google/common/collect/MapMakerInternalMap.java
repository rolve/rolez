/*
 * Copyright (C) 2009 The Guava Authors Licensed under the Apache License,
...
/*
 * Modified to play nice with JPF: Removed java.util.logging references.
 */
...
class MapMakerInternalMap<K, V> extends AbstractMap<K, V> implements
     ...
     * non-blocking reads and concurrent writes across different segments. If a
     * maximum size is specified, a best-effort bounding is performed per
     * segment, using a page-replacement algorithm to determine which entries to
     * evict when the capacity has been exceeded. The page replacement
     * algorithm's data structures are kept casually consistent with the map.
     * The ordering of writes to a segment is sequentially consistent. An update
     * to the map and recording of reads may not be immediately reflected on the
     * algorithm's data structures. These structures are guarded by a lock and
     * operations are applied in batches to avoid lock contention. The penalty
     * of applying the batches is spread across threads so that the amortized
     * cost is slightly higher than performing just the operation without
     * enforcing the capacity constraint. This implementation uses a per-segment
     * queue to record a memento of the additions, removals, and accesses that
     * were performed on the map. The queue is drained on writes and when it
     * exceeds its capacity threshold. The Least Recently Used page replacement
     * algorithm was chosen due to its simplicity, high hit rate, and ability to
     * be implemented with O(1) time complexity. The initial LRU implementation
     * operates per-segment rather than globally for increased implementation
     * simplicity. We expect the cache hit rate to be similar to that of a
    ...
    MapMakerInternalMap(final MapMaker builder) {
        ...
        if(evictsBySize()) {
            ...
            final int remainder = maximumSize % segmentCount;
    ...
    enum Strength {
        ...
        STRONG {
            ...
            <K, V> ValueReference<K, V> referenceValue(
                    ...
                    final ReferenceEntry<K, V> entry, final V value) {
        ...
        SOFT {
            ...
            <K, V> ValueReference<K, V> referenceValue(
                    final Segment<K, V> segment,
                    final ReferenceEntry<K, V> entry, final V value) {
                return new SoftValueReference<K, V>(
                        segment.valueReferenceQueue, value, entry);
            }
        ...
        WEAK {
            ...
            <K, V> ValueReference<K, V> referenceValue(
                    final Segment<K, V> segment,
                    final ReferenceEntry<K, V> entry, final V value) {
                return new WeakValueReference<K, V>(
                        segment.valueReferenceQueue, value, entry);
            }
    ...
    enum EntryFactory {
        STRONG {
            ...
            @Override
            <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> segment,
                    final K key, final int hash, final ReferenceEntry<K, V> next) {
        ...
        STRONG_EXPIRABLE {
            ...
            @Override
            <K, V> ReferenceEntry<K, V> copyEntry(final Segment<K, V> segment,
                    final ReferenceEntry<K, V> original,
                    final ReferenceEntry<K, V> newNext) {
                final ReferenceEntry<K, V> newEntry = super.copyEntry(segment,
                        original, newNext);
        ...
        WEAK {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> segment,
                    final K key, final int hash, final ReferenceEntry<K, V> next) {
                return new WeakEntry<K, V>(segment.keyReferenceQueue, key,
                        hash, next);
            }
        },
        WEAK_EXPIRABLE {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> segment,
                    final K key, final int hash, final ReferenceEntry<K, V> next) {
                return new WeakExpirableEntry<K, V>(segment.keyReferenceQueue,
                        key, hash, next);
            }
        ...
        WEAK_EVICTABLE {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> segment,
                    final K key, final int hash, final ReferenceEntry<K, V> next) {
                return new WeakEvictableEntry<K, V>(segment.keyReferenceQueue,
                        key, hash, next);
            }
        ...
        WEAK_EXPIRABLE_EVICTABLE {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> segment,
                    final K key, final int hash, final ReferenceEntry<K, V> next) {
                return new WeakExpirableEvictableEntry<K, V>(
        ...
        static EntryFactory getFactory(final Strength keyStrength,
                final boolean expireAfterWrite, final boolean evictsBySize) {
            final int flags = (expireAfterWrite ? EXPIRABLE_MASK : 0)
         ...
         */
        abstract <K, V> ReferenceEntry<K, V> newEntry(Segment<K, V> segment,
        ...
        <K, V> ReferenceEntry<K, V> copyEntry(final Segment<K, V> segment,
                final ReferenceEntry<K, V> original,
                final ReferenceEntry<K, V> newNext) {
            return newEntry(segment, original.getKey(), original.getHash(),
                    newNext);
        }
    ...
    interface ValueReference<K, V> {
         ...
         *             if the computing thread throws an exception
         */
         ...
         */
        ValueReference<K, V> copyFor(ReferenceQueue<V> queue, V value,
         ...
         * 
         * @param newValue
         *            the new value reference which will replace this one; this
         *            is only used during computation to immediately notify
         *            blocked threads of the new value
         */
        void clear(ValueReference<K, V> newValue);
        
        /**
         * Returns {@code true} if the value type is a computing reference
         * (regardless of whether or not computation has completed). This is
         * necessary to distiguish between partially-collected entries and
         * computing entries, which need to be cleaned up differently.
         */
    ...
    static final ValueReference<Object, Object> UNSET = new ValueReference<Object, Object>() {
        ...
        public ValueReference<Object, Object> copyFor(
                final ReferenceQueue<Object> queue, final Object value,
                final ReferenceEntry<Object, Object> entry) {
            return this;
        }
     ...
     * states: Valid: - Live: valid key/value are set - Computing: computation
     * is pending Invalid: - Expired: time expired (key/value may still be set)
    ...
    interface ReferenceEntry<K, V> {
        ...
        /*
         * Used by entries that are expirable. Expirable entries are maintained
    ...
    private enum NullEntry implements ReferenceEntry<Object, Object> {
        ...
        public void setValueReference(
                final ValueReference<Object, Object> valueReference) {}
        ...
        public void setPreviousExpirable(
                final ReferenceEntry<Object, Object> previous) {}
    ...
    abstract static class AbstractReferenceEntry<K, V> implements
            ReferenceEntry<K, V> {
    ...
    static final class StrongExpirableEntry<K, V> extends StrongEntry<K, V>
        ...
        StrongExpirableEntry(final K key, final int hash,
                final ReferenceEntry<K, V> next) {
            super(key, hash, next);
        }
    ...
    static final class StrongEvictableEntry<K, V> extends StrongEntry<K, V>
        ...
        StrongEvictableEntry(final K key, final int hash,
                final ReferenceEntry<K, V> next) {
            super(key, hash, next);
        }
    ...
    static final class StrongExpirableEvictableEntry<K, V> extends
            StrongEntry<K, V> implements ReferenceEntry<K, V> {
        
        StrongExpirableEvictableEntry(final K key, final int hash,
                final ReferenceEntry<K, V> next) {
            super(key, hash, next);
        }
     ...
     */
    static class SoftEntry<K, V> extends SoftReference<K> implements
            ReferenceEntry<K, V> {
        
        SoftEntry(final ReferenceQueue<K> queue, final K key, final int hash,
                final ReferenceEntry<K, V> next) {
            super(key, queue);
            this.hash = hash;
            this.next = next;
    ...
    static final class SoftExpirableEntry<K, V> extends SoftEntry<K, V>
        ...
        SoftExpirableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
    ...
    static final class SoftEvictableEntry<K, V> extends SoftEntry<K, V>
        ...
        SoftEvictableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
        }
    ...
    static final class SoftExpirableEvictableEntry<K, V> extends
            SoftEntry<K, V> implements ReferenceEntry<K, V> {
        
        SoftExpirableEvictableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
        }
     ...
     */
    static class WeakEntry<K, V> extends WeakReference<K> implements
            ReferenceEntry<K, V> {
        
        WeakEntry(final ReferenceQueue<K> queue, final K key, final int hash,
                final ReferenceEntry<K, V> next) {
            super(key, queue);
            this.hash = hash;
            this.next = next;
    ...
    static final class WeakExpirableEntry<K, V> extends WeakEntry<K, V>
        ...
        WeakExpirableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
        }
    ...
    static final class WeakEvictableEntry<K, V> extends WeakEntry<K, V>
        ...
        WeakEvictableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
        }
    ...
    static final class WeakExpirableEvictableEntry<K, V> extends
            WeakEntry<K, V> implements ReferenceEntry<K, V> {
        
        WeakExpirableEvictableEntry(final ReferenceQueue<K> queue, final K key,
                final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
        }
    ...
    static final class WeakValueReference<K, V> extends WeakReference<V>
        ...
        WeakValueReference(final ReferenceQueue<V> queue, final V referent,
                final ReferenceEntry<K, V> entry) {
        ...
        @Override
        public ValueReference<K, V> copyFor(final ReferenceQueue<V> queue,
                final V value, final ReferenceEntry<K, V> entry) {
    ...
    static final class SoftValueReference<K, V> extends SoftReference<V>
        ...
        SoftValueReference(final ReferenceQueue<V> queue, final V referent,
                final ReferenceEntry<K, V> entry) {
            super(referent, queue);
            this.entry = entry;
        }
     ...
     */
    static final class StrongValueReference<K, V> implements
            ValueReference<K, V> {
    ...
    @VisibleForTesting
    ReferenceEntry<K, V> newEntry(final K key, final int hash,
            final ReferenceEntry<K, V> next) {
    ...
    @VisibleForTesting
    ReferenceEntry<K, V> copyEntry(final ReferenceEntry<K, V> original,
            final ReferenceEntry<K, V> newNext) {
    ...
    @VisibleForTesting
    ValueReference<K, V> newValueReference(final ReferenceEntry<K, V> entry,
            final V value) {
    ...
    int hash(final Object key) {
        final int h = keyEquivalence.hash(key);
        return rehash(h);
    }
    ...
    Segment<K, V> createSegment(final int initialCapacity,
     ...
     * invalid, partially-collected, computing, or expired. Unlike
     * {@link Segment#getLiveValue} this method does not attempt to clean up
     * stale entries.
     */
    ...
    static <K, V> void connectExpirables(final ReferenceEntry<K, V> previous,
            final ReferenceEntry<K, V> next) {
    ...
    void processPendingNotifications() {
        ...
        while((notification = removalNotificationQueue.poll()) != null) {
            try {
                ...
                // "Exception thrown by removal listener", e);
            }
        }
    }
    ...
    static <K, V> void connectEvictables(final ReferenceEntry<K, V> previous,
            final ReferenceEntry<K, V> next) {
    ...
    static class Segment<K, V> extends ReentrantLock {
         ...
         * (unsynchronized) read operations must first read the "count" field,
         * and should not look at table entries if it is 0. - All (synchronized)
         * write operations should write to the "count" field after structurally
         * changing any bin. The operations must not take any action that could
         * even momentarily cause a concurrent read operation to see
         * inconsistent data. This is made easier by the nature of the read
         * operations in Map. For example, no operation can reveal that the
         * table has grown but the threshold has not yet been updated, so there
         * are no atomicity requirements for this with respect to reads. As a
         * guide, all critical volatile reads and writes to the count field are
        ...
        Segment(final MapMakerInternalMap<K, V> map, final int initialCapacity,
                final int maxSegmentSize) {
            this.map = map;
            ...
            recencyQueue = (map.evictsBySize() || map.expiresAfterAccess()) ? new ConcurrentLinkedQueue<ReferenceEntry<K, V>>() : MapMakerInternalMap
        ...
        ReferenceEntry<K, V> copyEntry(final ReferenceEntry<K, V> original,
            ...
            final ValueReference<K, V> valueReference = original
                    .getValueReference();
            ...
            final ReferenceEntry<K, V> newEntry = map.entryFactory.copyEntry(
                    this, original, newNext);
            newEntry.setValueReference(valueReference.copyFor(
                    this.valueReferenceQueue, value, newEntry));
            return newEntry;
        }
        ...
        void setValue(final ReferenceEntry<K, V> entry, final V value) {
            final ValueReference<K, V> valueReference = map.valueStrength
                    .referenceValue(this, entry, value);
        ...
        void drainKeyReferenceQueue() {
            ...
            while((ref = keyReferenceQueue.poll()) != null) {
                @SuppressWarnings("unchecked") final ReferenceEntry<K, V> entry = (ReferenceEntry<K, V>) ref;
        ...
        void drainValueReferenceQueue() {
            ...
            while((ref = valueReferenceQueue.poll()) != null) {
                @SuppressWarnings("unchecked") final ValueReference<K, V> valueReference = (ValueReference<K, V>) ref;
        ...
        void recordWrite(final ReferenceEntry<K, V> entry) {
            ...
            if(map.expires()) {
                ...
                final long expiration = map.expiresAfterAccess() ? map.expireAfterAccessNanos : map.expireAfterWriteNanos;
        ...
        void recordExpirationTime(final ReferenceEntry<K, V> entry,
        ...
        void expireEntries() {
            ...
            final long now = map.ticker.read();
            ReferenceEntry<K, V> e;
        ...
        void enqueueNotification(final ReferenceEntry<K, V> entry,
                final RemovalCause cause) {
            enqueueNotification(entry.getKey(), entry.getHash(), entry
                    .getValueReference().get(), cause);
        }
        
        void enqueueNotification(final K key, final int hash, final V value,
                final RemovalCause cause) {
            if(map.removalNotificationQueue != DISCARDING_QUEUE) {
                final RemovalNotification<K, V> notification = new RemovalNotification<K, V>(
                        key, value, cause);
        ...
        boolean evictEntries() {
            if(map.evictsBySize() && count >= maxSegmentSize) {
                ...
                final ReferenceEntry<K, V> e = evictionQueue.remove();
        ...
        ReferenceEntry<K, V> getLiveEntry(final Object key, final int hash) {
            final ReferenceEntry<K, V> e = getEntry(key, hash);
            if(e == null) {
                return null;
            }
        ...
        V get(final Object key, final int hash) {
            try {
                final ReferenceEntry<K, V> e = getLiveEntry(key, hash);
                if(e == null) {
                    return null;
                }
                ...
                if(count != 0) { // read-volatile
                    ...
                    for(int i = 0; i < length; ++i) {
                        for(ReferenceEntry<K, V> e = table.get(i); e != null; e = e
                                .getNext()) {
                            final V entryValue = getLiveValue(e);
                            if(entryValue == null) {
                                continue;
                            }
                            if(map.valueEquivalence.equivalent(value,
                                    entryValue)) {
                                return true;
                            }
                        }
        ...
        V put(final K key, final int hash, final V value,
            ...
            try {
                ...
                final ReferenceEntry<K, V> first = table.get(index);
                ...
                for(ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
                    ...
                    if(e.getHash() == hash && entryKey != null
                        ...
                        final ValueReference<K, V> valueReference = e
                                .getValueReference();
                        ...
                        if(entryValue == null) {
                            ...
                            else if(evictEntries()) { // evictEntries after
                                                      // setting new value
                                newCount = this.count + 1;
                            }
                ...
                // Create a new entry.
                ++modCount;
                final ReferenceEntry<K, V> newEntry = newEntry(key, hash, first);
        ...
        void expand() {
            final AtomicReferenceArray<ReferenceEntry<K, V>> oldTable = table;
            ...
            int newCount = count;
            final AtomicReferenceArray<ReferenceEntry<K, V>> newTable = newEntryArray(oldCapacity << 1);
                ...
                if(head != null) {
                    ...
                    if(next == null) {
                        ...
                        for(ReferenceEntry<K, V> e = head; e != tail; e = e
                                .getNext()) {
                            final int newIndex = e.getHash() & newMask;
                            final ReferenceEntry<K, V> newNext = newTable
                                    .get(newIndex);
                            final ReferenceEntry<K, V> newFirst = copyEntry(e,
                                    newNext);
                            if(newFirst != null) {
                    ...
                    if(e.getHash() == hash && entryKey != null
                        ...
                        if(entryValue == null) {
                            if(isCollected(valueReference)) {
                                int newCount = this.count - 1;
                                ++modCount;
                                enqueueNotification(entryKey, hash, entryValue,
                                        RemovalCause.COLLECTED);
                                final ReferenceEntry<K, V> newFirst = removeFromChain(
                                        first, e);
                                newCount = this.count - 1;
                        ...
                        if(map.valueEquivalence
                                .equivalent(oldValue, entryValue)) {
                            ++modCount;
                            enqueueNotification(key, hash, entryValue,
                                    RemovalCause.REPLACED);
                            setValue(e, newValue);
                ...
                try {
                    ...
                    if(map.removalNotificationQueue != DISCARDING_QUEUE) {
                        for(int i = 0; i < table.length(); ++i) {
                            for(ReferenceEntry<K, V> e = table.get(i); e != null; e = e
                                    .getNext()) {
                                // Computing references aren't actually in the
                                // map yet.
                                if(!e.getValueReference()
        ...
        ReferenceEntry<K, V> removeFromChain(final ReferenceEntry<K, V> first,
                final ReferenceEntry<K, V> entry) {
            ...
            for(ReferenceEntry<K, V> e = first; e != entry; e = e.getNext()) {
                final ReferenceEntry<K, V> next = copyEntry(e, newFirst);
                if(next != null) {
                    newFirst = next;
                }
                else {
        ...
        boolean reclaimKey(final ReferenceEntry<K, V> entry, final int hash) {
            ...
            try {
                ...
                for(ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
                    if(e == entry) {
                        ++modCount;
                        enqueueNotification(e.getKey(), hash, e
         ...
         */
        boolean reclaimValue(final K key, final int hash,
         ...
         */
        boolean clearValue(final K key, final int hash,
                final ValueReference<K, V> valueReference) {
            lock();
            try {
                final AtomicReferenceArray<ReferenceEntry<K, V>> table = this.table;
        ...
        boolean removeEntry(final ReferenceEntry<K, V> entry, final int hash,
                final RemovalCause cause) {
            int newCount = this.count - 1;
            final AtomicReferenceArray<ReferenceEntry<K, V>> table = this.table;
            final int index = hash & (table.length() - 1);
         ...
         * meaning that the value is null and it is not computing.
         */
        ...
        /**
         * Gets the value from an entry. Returns {@code null} if the entry is
         * invalid, partially-collected, computing, or expired.
         */
        V getLiveValue(final ReferenceEntry<K, V> entry) {
            if(entry.getKey() == null) {
                tryDrainReferenceQueues();
     ...
     * elements in the middle of the queue as part of copyEvictableEntry, and
     * (2) the contains method is highly optimized for the current model.
     */
    static final class EvictionQueue<K, V> extends
            AbstractQueue<ReferenceEntry<K, V>> {
        ...
        public boolean offer(final ReferenceEntry<K, V> entry) {
            // unlink
            connectEvictables(entry.getPreviousEvictable(), entry
                    .getNextEvictable());
        ...
        public ReferenceEntry<K, V> peek() {
            final ReferenceEntry<K, V> next = head.getNextEvictable();
        ...
        public boolean remove(final Object o) {
            final ReferenceEntry<K, V> e = (ReferenceEntry) o;
        ...
        public int size() {
            int size = 0;
            for(ReferenceEntry<K, V> e = head.getNextEvictable(); e != head; e = e
                    .getNextEvictable()) {
                size++;
            }
            return size;
        ...
        public Iterator<ReferenceEntry<K, V>> iterator() {
            return new AbstractSequentialIterator<ReferenceEntry<K, V>>(peek()) {
                ...
                protected ReferenceEntry<K, V> computeNext(
                        final ReferenceEntry<K, V> previous) {
                    final ReferenceEntry<K, V> next = previous
                            .getNextEvictable();
                    return (next == head) ? null : next;
                }
            };
     ...
     * (2) the contains method is highly optimized for the current model.
     */
    static final class ExpirationQueue<K, V> extends
            AbstractQueue<ReferenceEntry<K, V>> {
        ...
        public boolean offer(final ReferenceEntry<K, V> entry) {
            // unlink
            connectExpirables(entry.getPreviousExpirable(), entry
                    .getNextExpirable());
        ...
        @Override
        public ReferenceEntry<K, V> peek() {
            final ReferenceEntry<K, V> next = head.getNextExpirable();
            return (next == head) ? null : next;
        }
        ...
        public int size() {
            int size = 0;
            for(ReferenceEntry<K, V> e = head.getNextExpirable(); e != head; e = e
                    .getNextExpirable()) {
                size++;
            }
            return size;
    ...
    static final class CleanupMapTask implements Runnable {
        ...
        @Override
        public void run() {
            final MapMakerInternalMap<?, ?> map = mapReference.get();
            if(map == null) {
     ...
     * computing, expired, or partially collected. Does not impact recency
     * ordering.
     */
    ...
    public boolean containsValue(final Object value) {
        ...
        for(int i = 0; i < CONTAINS_VALUE_RETRIES; i++) {
            ...
            for(final Segment<K, V> segment : segments) {
                ...
                @SuppressWarnings({"UnusedDeclaration", "unused"}) final int c = segment.count; // read-volatile
                
                final AtomicReferenceArray<ReferenceEntry<K, V>> table = segment.table;
                for(int j = 0; j < table.length(); j++) {
                    for(ReferenceEntry<K, V> e = table.get(j); e != null; e = e
                            .getNext()) {
    ...
    abstract class HashIterator<E> implements Iterator<E> {
        ...
        boolean advanceTo(final ReferenceEntry<K, V> entry) {
            try {
                ...
                final V value = getLiveValue(entry);
                if(value != null) {
    ...
    final class WriteThroughEntry extends AbstractMapEntry<K, V> {
        ...
        public V setValue(final V newValue) {
            final V oldValue = put(key, newValue);
    ...
    final class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        public boolean remove(final Object o) {
            ...
            final Object key = e.getKey();
            return key != null
                    && MapMakerInternalMap.this.remove(key, e.getValue());
        }
    ...
    abstract static class AbstractSerializationProxy<K, V> extends
        ...
        AbstractSerializationProxy(final Strength keyStrength,
                final Strength valueStrength,
                final Equivalence<Object> keyEquivalence,
                final Equivalence<Object> valueEquivalence,
                final long expireAfterWriteNanos,
                final long expireAfterAccessNanos, final int maximumSize,
                final int concurrencyLevel,
                final RemovalListener<? super K, ? super V> removalListener,
        ...
        MapMaker readMapMaker(final ObjectInputStream in) throws IOException {
            ...
            final MapMaker mapMaker = new MapMaker().initialCapacity(size)
                    .setKeyStrength(keyStrength)
                    .setValueStrength(valueStrength).keyEquivalence(
                            keyEquivalence).concurrencyLevel(concurrencyLevel);
        ...
        @SuppressWarnings("unchecked")
        void readEntries(final ObjectInputStream in) throws IOException,
     ...
     * must be able to behave as the map itself.
     */
    private static final class SerializationProxy<K, V> extends
            AbstractSerializationProxy<K, V> {
        
        private static final long serialVersionUID = 3;
        
        SerializationProxy(final Strength keyStrength,
                final Strength valueStrength,
                final Equivalence<Object> keyEquivalence,
                final Equivalence<Object> valueEquivalence,
                final long expireAfterWriteNanos,
            ...
            super(keyStrength, valueStrength, keyEquivalence, valueEquivalence,
                    ...
                    concurrencyLevel, removalListener, delegate);
        }
        ...
        private void readObject(final ObjectInputStream in) throws IOException,
            ...
            final MapMaker mapMaker = readMapMaker(in);
