...
class InstrGenerator {
    ...
    def CharSequence generate(Stmt it) { generateStmt }
    ...
    def generateWithTryCatch(Block it, boolean forceTry) {
    ...
    def CharSequence generate(Expr it) { generateExpr }
    ...
    private def genCoerced(Expr it) {
        ...
        if(paramType instanceof JvmArrayType)
            '''«jvmGuardedArrayClassName».unwrap(«genNested», «paramType.generate».class)'''
        else
            generate
    }
    ...
    private def generate(JvmArrayType it) { toString.substring(14) } // IMPROVE: A little less magic, a little more robustness, please
    ...
    private def dispatch generateExpr(The it) '''«classRef.generate».INSTANCE'''
    
    private def dispatch generateExpr(Parenthesized it) { expr.generate }
    ...
    private def genNested(Expr it) { switch(it) {
        Assignment: generate
        BinaryExpr: '''(«generate»)'''
        UnaryExpr : '''(«generate»)'''
        default   : generate
    ...
    def generate(VarKind it) { if(it == VAL) "final " }
}