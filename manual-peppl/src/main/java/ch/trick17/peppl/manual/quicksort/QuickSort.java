...
public class QuickSort implements Runnable {
    ...
    public class SortTask implements Runnable {
        ...
        public void sort() {
            ...
            if(s.range.size() > 2) {
                ...
                if(s.range.begin < r) {
                    ...
                    if(left.range.size() >= MIN_TASK_SIZE) {
                        left.pass();
                        SYSTEM.run(task);
                    }
                    else
                ...
                if(l < s.range.end - 1) {
                    final IntSlice right = s.slice(l, s.range.end, 1);
                    final SortTask task = new SortTask(right);
                    if(right.range.size() >= MIN_TASK_SIZE) {
                        right.pass();
                        SYSTEM.run(task);
                    }
                    else
        ...
        private int pivot(final @_UnguardedRead IntSlice slice) {
            ...
            assert !slice.range.isEmpty();
            final int l = slice.data[slice.range.begin];
            final int m =
                    slice.data[slice.range.begin + slice.range.size() / 2];
